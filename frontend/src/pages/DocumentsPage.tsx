...
const COLUMNS = [
  ...
  { key: 'doc_type', label: 'Document Type', width: '160px' },
  { key: 'customer_name', label: 'Customer Name', width: 'auto' },
  { key: 'issue_date', label: 'Issue Date', width: '120px' },
  { key: 'expiry_date', label: 'Expiry Date', width: '120px' },
  { key: 'days_until_expiry', label: 'Days Until Expiry', width: '140px' },
...
export function DocumentsPage() {
  ...
  // Sorting
  const [sort, setSort] = useState<SortState>({ column: 'expiry_date', order: 'asc' });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (docTypeFilter.length > 0) params.set('doc_type', docTypeFilter.join(','));
  ...
  }, [search, statusFilter, docTypeFilter, sort]);
  ...
  if (error) {
    return (
      <div data-testid="page-documents" className="space-y-6">
  ...
  return (
    <div data-testid="page-documents" className="space-y-0">
      ...
      >
        <div className="flex items-center gap-3 flex-wrap">
          {/* Search Input */}
          <div className="relative flex-1 min-w-[240px] max-w-[360px]" data-testid="document-search-container">
            <Search
          ...
          <MultiSelectFilter
            label="Status"
            options={['Verified', 'Pending', 'Expired', 'Rejected']}
          ...
          <MultiSelectFilter
            ...
            isOpen={openFilter === 'doc_type'}
            onToggle={() => setOpenFilter(openFilter === 'doc_type' ? null : 'doc_type')}
            testId="filter-doc-type"
          />
      ...
      >
        {loading ? (
          ...
          <div className="p-12 text-center" data-testid="empty-state">
            <FileText className="mx-auto mb-3 text-muted-foreground" style={{ width: 32, height: 32, opacity: 0.5 }} />
          ...
          <>
                  ...
                  {paginatedDocuments.map((doc) => {
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          backgroundColor: isExpired ? 'rgba(239, 68, 68, 0.03)' : undefined,
                          boxShadow: isExpired ? 'inset 3px 0 0 0 #ef4444' : undefined,
                        }}
                        onMouseEnter={(e) => {
                          (e.currentTarget as HTMLElement).style.backgroundColor = '#eff6ff';
                        }}
                        onMouseLeave={(e) => {
                          (e.currentTarget as HTMLElement).style.backgroundColor = isExpired ? 'rgba(239, 68, 68, 0.03)' : '';
                        }}
                      >
                        <td
                          style={{
                        ...
                        >
                          {daysUntilExpiry === null ? (
                            ...
                            <span
                              className="inline-flex items-center text-xs font-bold uppercase rounded-full whitespace-nowrap"
                              style={{
                            ...
                            <span
                              className="text-sm"
                              style={{
                                color: isExpiringSoon ? '#dc2626' : '#475569',
                        ...
                        >
                          <StatusBadge
                            ...
                            testId={`status-badge-${doc.document_id}`}
                          />
                        </td>
                      </tr>
                    );
            ...
            >
              ...
              >
                Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, documents.length)} of{' '}
